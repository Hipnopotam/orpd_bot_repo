...
def result_fun(message):
    ...
    resultNTD = ntd_fun_result(rabD, obem, rabT, tKip, sreda, sos_type)
...
def ntd_fun_result(rabD, obem, rabT, tKip, sreda, sos_type):
    if rabD>0.07 and obem<=0.025 and obem*rabD<=0.02:
        ntdMess = f'На {sos_type} распространяется ИТНЭ-93. \nСогласно пункта 5 ж) ФНП ОРПД - сосуд не подпадает под ФНП.'
        ntd = 'ИТНЭ'
    elif rabD>0.07 and rabT>tKip and (sreda=='Жидкость' or sreda=='жидкость'):
        ntdMess = f'На {sos_type} распространяется ФНП ОРПД - ст.2 в).'
        ntd = 'ФНП'
    elif rabD>0.07 and sreda=='газ':
        ntdMess = f'На {sos_type} распространяется ФНП ОРПД - ст.2 а).'
        ntd = 'ФНП'
    elif obem*rabD>0.02 and rabD>0.07 and obem<=0.025 and rabT>tKip:
        ntdMess = f'На {sos_type} распространяется ФНП ОРПД - ст.5 ж).'
        ntd = 'ФНП'
    elif rabD>0.07 and sreda=='вода' and rabT>115:
        ntdMess = f'На {sos_type} распространяется ФНП ОРПД - ст.2 б).'
        ntd = 'ФНП'
    elif rabD<=0.07:
        ntdMess = f'На {sos_type} распространяется РУА-93.'
        ntd = 'РУА'
    else:
        ntdMess = f'На {sos_type} распространяется ИТНЭ-93.'
        ntd = 'ИТНЭ'
...
def rtn_fun(message):
    ...
    zapros = f'SELECT rabT, obem, ntd, rabD, srTRTS, sosType FROM ras4et WHERE telegram_id={message.chat.id}'
    ...
    srTRTS = str(zap[0][4])
    sos_type = str(zap[0][5])
    # print(ntd,srTRTS,rabT,rabD,obem, 'FNP_FUN') #ФНП 0.1 202.0 1.0 3.0 FNP_FUN
    result = fnp_fun(ntd, srTRTS, rabT, rabD, obem, sos_type)
    rtnMess = result
...
def fnp_fun(ntd, srTRTS, rabT, rabD, obem, sos_type):
    if ntd=='ФНП':
        if srTRTS=='1 группа' and (rabT>200 or rabD*obem>0.05):
            rtn = 1
            rtnMess = f'{sos_type} необходимо ставить на учет в РТН.'
        elif srTRTS=='2 группа' and (rabT>200 or rabD*obem>1):
            rtn = 1
            rtnMess = f'{sos_type} необходимо ставить на учет в РТН.'
        else:
            rtn = 0
            rtnMess = f'{sos_type} на учет в РТН ставить не надо. Статья 223 а).'
    else:
        rtn = 0
        rtnMess = f'{sos_type} на учет в РТН ставить не надо.'
    rtnMess = rtnMess+' '
    result = rtnMess
    return result
